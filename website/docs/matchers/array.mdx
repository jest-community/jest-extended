...
Use `.toBeArrayOfUniqueItems` when checking if a value is an `Array` with all unique items (no duplicates).

<TestFile name="toBeArrayOfUniqueItems">
  {`test('passes when value is an array with unique items', () => {
